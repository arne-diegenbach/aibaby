...
const Spec kSpecs[] = {
    ...
    {"ctxsrc", 600000, Expect::kOpen, Tier::kLong,
     "derived: vocallearn's 2800-tick trial, and it needs at least 16 balanced\n"
     "  trials for a held-out split. The last piece of the Area X architecture:\n"
     "  areax has the HOST write the context slice, and this asks whether the\n"
     "  creature can derive it from what it heard AT THE MOMENT REWARD LANDS.\n"
     "  No learning. Bar is 0.75, derived from how a wrong index writes the\n"
     "  other context's table"},
    ...
    {"orthoname", 3400000, Expect::kOpen, Tier::kLong,
     "orthovocab showed two SEQUENTIAL lessons on different axes coexist (77%\n"
     "  less damage, 10.7 SE). Naming is SIMULTANEOUS and conditional. Does\n"
     "  spreading a vocabulary across articulators make it reachable? collide\n"
    ...
    {"absbar", 3400000, Expect::kOpen, Tier::kLong,
     "travelsweep found the shipped bar BIT-IDENTICALLY blind to target\n"
     "  distance past ~0.6 log units: it is an EMA of the creature's own error,\n"
     "  so a further target only adds a constant, which cancels. Can any bar\n"
     "  ask? Derived first: linear is PROVABLY invariant (the constant\n"
     "  multiplies E[u]=0), quadratic has a term scaling with distance, a hard\n"
    ...
    {"framehold", 1200000, Expect::kOpen, Tier::kTeach,
     "DOES THE RHYTHM OUTLAST THE CAREGIVER? framecopy looked spectacular and was\n"
     "  the shipped listening reflex: the baby stops babbling while it hears, so its\n"
     "  voice squares up at the caregiver's rate. An interrupt, not a frame. This\n"
    ...
    {"salience", 600000, Expect::kOpen, Tier::kLong,
     ...
     "  CPG, rewarding summed spectrotemporal TRANSIENTS against a staircase on\n"
     "  the creature's own hit rate. A reward can only grade what it can tell\n"
     "  apart, so: do this creature's vocalisations VARY on that measure? Summed\n"
     "  |frame-to-frame change| of the mel spectrogram on its own voice, with a\n"
     "  caregiver arm for scale and a silence arm for the floor. 12 seeds"},
    ...
    {"glide", 1200000, Expect::kOpen, Tier::kTeach,
     ...
     "  has EVER changed while sounding: a long line of work asked whether the brain\n"
     "  can GENERATE a sequence and none asked whether it can FOLLOW one. The rate is\n"
     "  DERIVED -- the formant pole is tau 800 ms, so a human diphthong passes at\n"
    ...
    {"orthovocab", 5600000, Expect::kOpen, Tier::kLong,
     ...
     "  decay together). So does it need AVOIDING rather than defending?\n"
     "  `capacity` holds two ORTHOGONAL lessons at 0.84 against 0.22, never\n"
     "  tested inside the retention protocol. Lesson A identical in every arm;\n"
     "  only the second lesson's AXIS differs -- collide moves F1, ortho moves\n"
     "  F2, same log-distance. Per-axis scoring, since the joint metric pins\n"
     "  both formants and cannot express orthogonality. 36 seeds"},
    ...
    {"ctxfour", 3400000, Expect::kOpen, Tier::kTeach,
     ...
     "  mappings on one reward channel. Oracle arm only -- if a perfect index\n"
     "  cannot carry four, the creature's own cannot either. Refuses unless\n"
     "  ctx_match reads 1.000, which is the k-way assignment's own check"},
